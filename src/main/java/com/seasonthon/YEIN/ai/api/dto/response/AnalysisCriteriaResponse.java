...
public record AnalysisCriteriaResponse(
        ...
        @Schema(description = "분석 기준 설명", example = "손글씨 분석은 3가지 기준으로 평가됩니다:")
        String description,
        ...
        @Schema(description = "총점 설명", example = "총 100점 만점으로 평가되며, 정렬과 간격은 각 33점, 일관성은 34점입니다.")
        String totalScoreInfo

) {
    public static AnalysisCriteriaResponse createDefault() {
        return new AnalysisCriteriaResponse(
                "손글씨 분석은 3가지 기준으로 평가됩니다:",
                new CriteriaItemResponse("정렬", "글자들이 베이스라인에 정렬된 정도", 33),
                new CriteriaItemResponse("간격", "글자 사이의 간격이 일정한 정도", 33),
                new CriteriaItemResponse("일관성", "글자 크기가 균등한 정도", 34),
                "총 100점 만점으로 평가되며, 정렬과 간격은 각 33점, 일관성은 34점입니다."
        );
    }
}
